...
class CurrentConditionsHandler:
    ...
    async def process_data(self, full_data):
        ...
        try:
            ...
            if (
                fields.get("air_temperature") is not None
                and fields.get("relative_humidity") is not None
            ):
                weather_data = {
                ...
                try:
                    ...
                    logger_CurrentConditionsHandler.warning(
                        f"Error calculating weather metrics: {e}"
                    )
            ...
            tags = {
                ...
                "station_id": str(
                    station_info.get("station_id", metadata.get("station_id", "unknown"))
                ),
            }
            ...
            if not fields:
                logger_CurrentConditionsHandler.debug(
                    "No valid fields after filtering None values, skipping"
                )
                return
